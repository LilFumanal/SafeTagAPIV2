...
class PractitionerControllerTest {
    ...
    void searchPractitioners_withValidParams_returnsPageOfPractitioners() throws Exception {
        ...
        PractitionerDTO practitioner1 = new PractitionerDTO(
                "123454", "Jean Dupont", List.of("10"), List.of("SM34"), List.of(), "Libéral");
        ...
        // When & Then
        mockMvc.perform(get("/practitioners/search")
                        .param("name", name)
                ...
                .andExpect(jsonPath("$.content[0].rppsId").value("12345")) // Vérifie le contenu de la réponse (JSON Path)
    ...
    void getPractitioner_withExistingRppsId_returnsPractitionerDTO() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/practitioners/{rppsId}", rppsId)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
